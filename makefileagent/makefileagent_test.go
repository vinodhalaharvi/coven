...
func TestRoleString_HasCanonicalTargets(t *testing.T) {
	// Soft check — these are the targets a typical Go Makefile would
	// have, and the role string mentions them as "canonical."
	must := []string{
		"build", "run", "test", "vet", "tidy", "fmt", "clean", "help",
		"go.mod", "final authority", "dormant",
	}
	for _, frag := range must {
		if !strings.Contains(Role, frag) {
			t.Errorf("Role missing %q", frag)
		}
	}
}
...
func TestAgent_RunWakesOnceAndBlocks(t *testing.T) {
	...
	sender := func(ctx context.Context, system string, conv []llm.Message, tools []llm.ToolSpec) (llm.Message, llm.StopReason, error) {
		...
		return llm.Message{
			...
			Blocks: []llm.Block{{Text: "proposed Makefile"}},
		}, llm.StopEndTurn, nil
	}
	...
	for _, frag := range []string{"makefile", "go.mod"} {
		if !strings.Contains(strings.ToLower(observations[0]), frag) {
