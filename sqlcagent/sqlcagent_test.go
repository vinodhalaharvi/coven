...
func TestFormatObservation(t *testing.T) {
	...
	if got := formatObservation([]string{"queries/users.sql"}); !strings.Contains(got, "users.sql") {
		t.Errorf("single: %q", got)
	}
	got := formatObservation([]string{"a.sql", "b.sql", "a.sql"})
...
func TestAgent_BootstrapWakesOnStartup(t *testing.T) {
	...
	sender := func(ctx context.Context, system string, conv []llm.Message, tools []llm.ToolSpec) (llm.Message, llm.StopReason, error) {
		...
		return llm.Message{
			...
			Blocks: []llm.Block{{Text: "no sqlc config, nothing to do"}},
		}, llm.StopEndTurn, nil
	}
...
func TestAgent_WakesOnSqlEdit(t *testing.T) {
	root := t.TempDir()
	if r, err := filepath.EvalSymlinks(root); err == nil {
		root = r
	}
	...
	sender := func(ctx context.Context, system string, conv []llm.Message, tools []llm.ToolSpec) (llm.Message, llm.StopReason, error) {
		...
		wakes++
		if len(conv) > 0 {
			for _, b := range conv[len(conv)-1].Blocks {
				if b.Text != "" {
	...
	board.Post("k1", fsmonitor.FileChangeFact{
		ChangedFiles: []string{filepath.Join(root, "queries", "users.sql")},
		ChangedAt:    time.Now(),
	}, "test")
	...
	for _, o := range observations {
		if strings.Contains(o, "users.sql") {
			found = true
		}
	}
	if !found {
		t.Errorf("expected observation to mention users.sql; got %v", observations)
	}
}
